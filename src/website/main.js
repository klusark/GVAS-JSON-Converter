const { convertJsonToSav, convertSavToJson, convertRawToSav } = require("../converter/converter");
const SavReader = require("../converter/sav-reader");
...
function createTileBank() {
    ...
    tileTypes.forEach(tileData => {
        ...
        if (tileData.rotatable) {
            tile.classList.add(selectedDirection);
        }
        ...
        tilesToMove.forEach(tile => {
            ...
            if (newRow >= 0 && newRow < 40 && newCol >= 0 && newCol < 50) {
                if (tile.tileType) {
                    ...
                    if (typeof tile.tileType === 'object') {
                        // New format with direction
                        cell.className = `grid-cell tile-${tile.tileType.type}`;
                        if (tile.tileType.direction) {
                            cell.classList.add(tile.tileType.direction);
                        }
                        grid[newRow][newCol] = tile.tileType;
                    } else {
                        // Legacy format
                        cell.className = `grid-cell tile-${tile.tileType}`;
                        grid[newRow][newCol] = { type: tile.tileType };
                    }
...
export function importLevel() {
    ...
    input.onchange = (e) => {
        ...
        if (file) {
            ...
            reader.onload = (event) => {
                try {
                    levelData = new SavReader(event.target.result).readWholeBuffer();
